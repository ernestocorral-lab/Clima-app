...
const DAY_LABEL_FONT_SIZE = 13;
const MAX_DOT_RADIUS = 6;
const MIN_DOT_RADIUS = 6;
...
export function buildWidgetChartSvg(
  ...
  const maxPath = buildSmoothPath(maxPeakPoints);
  const minPath = showMinEnvelope ? buildSmoothPath(minPeakPoints) : '';

  const maxLabels = maxPeakPoints
    .map((point) => {
      const labelY = Math.max(MAX_LABEL_FONT_SIZE + 2, point.y - 10);
  ...
  const minLabels = minPeakPoints
    .map((point) => {
      const labelY = Math.min(plotHeight - paddingBottom + 4, point.y + MIN_LABEL_FONT_SIZE + 4);
  ...
  const dayLabels = getWeekDayMarkers(points)
    .map((marker) => {
      ...
      return `<text x="${labelX.toFixed(1)}" y="${dayLabelY.toFixed(1)}" fill="${LABEL_COLOR}" font-size="${DAY_LABEL_FONT_SIZE}" text-anchor="${anchor}" font-family="sans-serif" font-weight="600">${marker.label}</text>`;
    })
    .join('');
